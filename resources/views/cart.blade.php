            ...
            <div class="col-lg-8">
                ...
                <div class="card">
                    <div class="card-header bg-light">
                        <h5 class="mb-0">Cart Items ({{ count($books) }})</h5>
                    </div>
                    <div class="card-body">
                        @foreach($books as $book)
                        <div class="row align-items-center mb-4 pb-4 border-bottom">
                            <div class="col-md-2">
                                <img src="{{ $book->cover_image ? asset('storage/' . $book->cover_image) : 'https://via.placeholder.com/80x100/4A6572/FFFFFF?text=No+Image' }}" 
                                     alt="Book Cover" class="img-fluid rounded shadow">
                            </div>
                            <div class="col-md-4">
                                <h5 class="mb-1">{{ $book->title }}</h5>
                                <p class="text-muted mb-1">by {{ $book->author }}</p>
                                <p class="text-success mb-0">${{ number_format($book->price, 2) }}</p>
                            </div>
                            <div class="col-md-3">
                                <form action="{{ route('cart.update') }}" method="POST" class="d-flex align-items-center">
                                    ...
                                    <input type="number" class="form-control form-control-sm" 
                                           name="quantity" value="{{ $book->cart_quantity }}" 
                                           min="1" max="{{ $book->quantity }}" style="width: 80px;">
                                    <button type="submit" class="btn btn-sm btn-outline-primary ms-2">
                                        <i class="fas fa-sync-alt"></i>
                                    </button>
                                </form>
                                <small class="text-muted">Max: {{ $book->quantity }} available</small>
                            </div>
                ...
                <div class="card">
                    ...
                    <div class="card-body">
                        <div class="d-flex justify-content-between mb-2">
                            <span>Subtotal ({{ count($books) }} items):</span>
